/* =============================================
   XPENSES GAME — App Logic v4 (Supabase + OAuth)
   ============================================= */
'use strict';
...
async function doLogin() {
  ...
  if (sb) {
    try {
      ...
      // Fall through to local auth if Supabase not fully configured
      console.warn('[doLogin] Supabase auth failed, trying local:', e.message);
...
async function doRegister() {
  ...
  if (sb) {
    try {
      ...
      console.warn('[doRegister] Supabase failed, using local:', e.message);
...
window.createGroup = function() {
  ...
  const g = DB.groups.create({ name, type, currency, palette }, State.currentUser.id);
  State.currentGroup = g;
  applyPalette(g.palette);
  closeModal();
  toast(`Grupo "${g.name}" creado 🎉`, 'success');
  showPage('group');
...
window.joinGroup = function() {
  ...
  if (!group) { errEl.textContent = 'No se encontró ningún grupo con ese ID'; errEl.classList.remove('hidden'); return; }
