...
async function doRegister() {
  ...
  if (sb) {
    try {
      ...
      const data = await SupabaseRepo.auth.signUp(email, password, { full_name: `${name} ${lastName}` });
      
      if (data.session) {
        // Email confirmation is disabled! User is already signed in.
        toast('¡Cuenta creada y confirmada automáticamente!', 'success');
        // Let onAuthStateChange handle showing the home page, just return.
        return;
      } else {
...
window.createGroup = async function() {
  ...
  if (sb) {
    try {
      ...
      toast('Error en la nube: ' + (e.message || e), 'error');
      // Stop the process completely so the user sees the error and we don't desync
      return; 
    }
  }
...
window.joinGroup = async function() {
  ...
  if (sb) {
    try {
      ...
      toast('Error uniéndose en la nube: ' + (e.message || e), 'error');
      return; // Stop local fallback
    }
  }
